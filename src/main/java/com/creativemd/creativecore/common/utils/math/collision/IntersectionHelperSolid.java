...
public class IntersectionHelperSolid {
	...
	public static List<Vector2f> cutMinMax(Axis one, Axis two, float minOne, float minTwo, float maxOne, float maxTwo, Vector3f[] corners) {
		...
		boolean clockwise = (VectorUtils.get(one, corners[0]) - originOne) * (VectorUtils.get(two, corners[2]) - originTwo) - (VectorUtils.get(two, corners[0]) - originTwo) * (VectorUtils.get(one, corners[2]) - originOne) > 0;
		if (allInside)
			if (clockwise)
