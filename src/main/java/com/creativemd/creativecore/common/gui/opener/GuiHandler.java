...
public class GuiHandler implements IGuiHandler {
    ...
    public Object getClientGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        ...
        case 0: // It's a block
            ...
            IBlockState state = world.getBlockState(pos);
            if (state.getBlock() instanceof IGuiCreator)
                return new GuiContainerSub(player, ((IGuiCreator) state.getBlock()).getGui(player, null, world, pos, state), ((IGuiCreator) state.getBlock())
                    .getContainer(player, null, world, pos, state));
        case 1: // It's an item
            ItemStack stack = player.getHeldItemMainhand();
            if (stack == null || !(stack.getItem() instanceof IGuiCreator))
                stack = player.getHeldItemOffhand();
            if (stack != null && stack.getItem() instanceof IGuiCreator)
                return new GuiContainerSub(player, ((IGuiCreator) stack.getItem()).getGui(player, stack, null, null, null), ((IGuiCreator) stack.getItem())
                    .getContainer(player, stack, null, null, null));
        }
        return null;
    }
