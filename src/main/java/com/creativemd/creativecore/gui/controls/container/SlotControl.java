...
public class SlotControl extends ContainerControl{
	...
	public void writeToNBTUpdate(NBTTagCompound nbt) {
		...
		if(slot.getHasStack())
		{
			...
			if(stack.stackSize > Byte.MAX_VALUE)
				nbt.setInteger("realCount", stack.stackSize);
	...
	public ItemStack putItemInSlot(Slot slot, ItemStack stack)
	{
		...
		if(canStack && slot.isItemValid(stack))
		{
			...
			if(slot.getHasStack())
				availableStackSize -= slot.getStack().stackSize;
			...
			if(additionalStackSize > 0)
			{
				...
				inSlot.stackSize = additionalStackSize;
				if(slot.getHasStack())
					inSlot.stackSize += slot.getStack().stackSize;
				slot.putStack(inSlot);
				
				stack.stackSize -= additionalStackSize;
	...
	public void mergeToOtherInventory(ItemStack toTransfer, boolean useEmptySlot)
	{
		for (int i = 0; i < getParent().controls.size(); i++) {
			if(getParent().controls.get(i) instanceof SlotControl)
			{
				...
				if(mergeSlot.inventory != slot.inventory && (useEmptySlot || mergeSlot.getHasStack()))
				{
	...
	public void transferIntoOtherInventory(boolean reverse, int amount)
	{
		if(slot.getHasStack())
		{
			...
			if(reverse)
			{
				int stackSizeLeft = Math.min(amount, stack.stackSize);
				for (int i = 0; i < getParent().controls.size(); i++) {
					if(getParent().controls.get(i) instanceof SlotControl)
					{
						Slot mergeSlot = ((SlotControl)getParent().controls.get(i)).slot;
						if(mergeSlot.inventory != slot.inventory)
						{
							...
							if(canStack && mergeSlot.getHasStack())
							{
								...
								mergeSlot.getStack().stackSize -= transfer;
	...
	public void onSlotClicked(int mouseButton, boolean shift, int scrolled)
	{
		...
		if(mouseButton == 0)
		{
			if(slot.canTakeStack(getPlayer()))
			{
				if(shift){
					...
					if(hand != null)
					{
						...
						int stackSize = Math.min(Math.min(slotItem.stackSize, 64), slotItem.stackSize);
						ItemStack newHand = slotItem.copy();
						newHand.stackSize = stackSize;
			...
			if(hand == null && slot.getHasStack() && getPlayer().capabilities.isCreativeMode)
			{
				...
				stack.stackSize = getItemStackLimit(stack);
