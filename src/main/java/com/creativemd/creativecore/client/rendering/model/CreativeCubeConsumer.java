...
public class CreativeCubeConsumer {
	...
	protected void processQuad() {
		 ...
		 * normal = quadData[normalIndex]; } else {
		 */
		...
		for (int v = 0; v < 4; v++) {
			...
			for (int e = 0; e < count; e++) {
				...
				case POSITION:
					// position adding moved to VertexBufferConsumer due to x and z not fitting
					// completely into a float
					/*
					 * float[] pos = new float[4]; System.arraycopy(position[v], 0, pos, 0,
					 * position[v].length); pos[0] += blockInfo.getBlockPos().getX(); pos[1] +=
					 * blockInfo.getBlockPos().getY(); pos[2] += blockInfo.getBlockPos().getZ();
					 */
