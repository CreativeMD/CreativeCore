...
public class QuadCache {
	...
	public void addCache(int index, float... cache/* , EnumUsage usage */) {
		...
		/*
		 * if(renderer.isColorDisabled() && format.getElement(index).getUsage() ==
		 * EnumUsage.COLOR) { cache = dummyColor; }
		 */
	 ...
	 * 
	 * public int index; public float[] cache; //public
	 * VertexFormatElement.EnumUsage usage;
	 * 
	 * public FloatCache(int index, float[] cache)//, EnumUsage usage) { this.index
	 * = index; this.cache = Arrays.copyOf(cache, cache.length); //this.usage =
	 * usage; }
	 * 
	 * }
	 */
